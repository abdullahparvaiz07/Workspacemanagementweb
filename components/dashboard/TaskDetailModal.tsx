...
import { TaskStatus, Comment } from '@/types';
import {
  X,
  CheckSquare,
  Clock,
...
export default function TaskDetailModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/50 backdrop-blur-sm overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl border border-stone-200 w-full max-w-3xl overflow-hidden my-8 animation-fade-in flex flex-col max-h-[90vh]">
        {/* Header */}
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-sm font-sans flex-1">
          {/* Title & Status Bar */}
          <div>
            <h2 className="font-serif text-2xl font-bold text-stone-900 mb-3">{task.title}</h2>
          ...
          {task.tags.length > 0 && (
            <div>
              <h4 className="text-xs font-semibold text-stone-400 uppercase tracking-wider mb-1.5">Tags</h4>
          ...
          <div>
            ...
            <div className="space-y-2 mb-3">
              {task.subtasks.map((st) => (
                <label
                  key={st.id}
                  className="flex items-center gap-3 p-3 rounded-xl bg-stone-50 hover:bg-stone-100/80 border border-stone-100 cursor-pointer transition-colors"
                >
                  <input
                    type="checkbox"
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-xl border border-stone-200 text-xs text-stone-800"
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    handleAddSubtask();
              ...
              <button
                ...
                className="px-3 py-2 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-xl text-xs font-medium transition-colors flex items-center gap-1"
              >
          ...
          <div className="pt-4 border-t border-stone-100">
            ...
            {permissions.canComment && (
              <form onSubmit={handlePostComment} className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="px-4 py-2.5 bg-stone-900 text-white rounded-xl text-xs font-medium hover:bg-stone-800 transition-all flex items-center gap-1.5 shadow-sm"
                >
